...
class InstallationTeamMember(models.Model):
    ...
    team_member_ids  = fields.Many2many('res.users',string="Members",domain=lambda self: [('id', 'in', self._get_group_inst_user_ids())])


    @api.model
    def _get_group_user_ids(self):
        group = self.env.ref('arm_customization.group_installation_manager')
        return group.users.ids if group else []

    @api.model
    def _get_group_inst_user_ids(self):
        group = self.env.ref('arm_customization.group_installation_user')
        return group.users.ids if group else []