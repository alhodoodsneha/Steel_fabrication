...
export class GanttRenderer extends Component {
    setup() {
        ...
        useBus(this.env.bus, "crate_link", this._onCreateLink.bind(this));
        useBus(this.env.bus, "delete_link", this._onDeleteLink.bind(this));
    ...
    async renderView(){
        ...
        console.log(self.titleContainerRef.el, this.props.model.gantt.date_display)
    ...
    _configGantt() {
        ...
        gantt.config.drag_links = this.props.model.metaData.showLinks === 'true' ?  true : false;
    ...
    _renderRows(){
        ...
        if (self.props.model.metaData.showLinks === 'true'){
            if (self.state.data.links.length > 0){
                build_links(self.state.data.links);
            }                
        }
    ...
    _configureGanttEvents(tasks, grouped_by, groups) {
        ...
        this.gantt_events.push(gantt.attachEvent("onAfterLinkAdd", function(id,item){     
            var crate_link = function (item) {                    
                self.env.bus.trigger('crate_link', {
                    link: item,
                    success: function (newID) {
                        if (newID){
                            var id  = uniqueId("gantt_link_");
        ...
        this.gantt_events.push(gantt.attachEvent("onAfterLinkDelete", function(id,item){
            var delete_link = function (item) {
                self.env.bus.trigger('delete_link', {
                    link: item,
                        ...
                        if (result){
                            ...
                            if (links.length > 0){
                                for(var i = 0, ilen = links.length; i < ilen; ++i){
                                    if (links[i].link_id === item.link_id){
        ...
        this.gantt_events.push(gantt.attachEvent("onBeforeLinkAdd", function(id,item){                
            ...
            if(sourceTask.parent != targetTask.parent || sourceTask.is_task != true || targetTask.is_task != true){
    ...
    _displayTask(task, readonly) {
        ...
        var form = self.dialog.add(FormViewDialog, {
            resModel: self.props.model.metaData.resModel,
        ...
        console.log("===fromview", form);
    }
    async _onCreateLink(event){
        var self = this;
        var item = event.detail;
        var linkObj = item.link;
        ...
        return await this.orm.call(self.props.model.metaData.linkModel, "create", [{
    ...
    async _onDeleteLink(event) {
        ...
        return await this.orm.call(self.props.model.metaData.linkModel, "unlink", [Id]).then(success, fail);;
    }
}

GanttRenderer.template = "web_gantt_compact_view_adv.ViewRenderer";
GanttRenderer.props = {
    model: { type: Object },
};
