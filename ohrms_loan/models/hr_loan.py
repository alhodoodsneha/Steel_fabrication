...
class HrLoan(models.Model):
    ...
    type_of_loan = fields.Selection([('medical','Medical'),('loan','Loan'),('salary_advance','Salary Advance')],string="Type")
    ...
    def action_compute_installment(self):
        ...
        for loan in self:
            ...
            if loan.emi_amount <= 0:
                raise UserError(_("EMI amount must be greater than zero."))
            total_period = int(loan.loan_amount / loan.emi_amount)
            amount_total = 0.0
            for i in range(1, total_period + 1):
                ...
                amount_total = amount_total + loan.emi_amount
            remaining_amount = loan.loan_amount - amount_total
...
class HrLoanLine(models.Model):
    ...
    payslip_id = fields.Many2one('hr.payslip', string="Payslip Ref.",
                                 help="Reference to the associated "
                                      "payslip, if any.")
    state = fields.Selection([('scheduled','Scheduled'),('hold','Hold')],string='Status',default='scheduled')


    def action_hold_loan(self):
        last_line = self.loan_id.loan_lines.sorted('date')[-1]
        new_line_date = last_line.date + relativedelta(months=1)
        self.state ='hold'
